...
export default async function DettaglioImpegnoPage({
  ...
  if (autoTerapiaId) {
    redirect(`/terapie/${autoTerapiaId}`)
  }
    ...
    <div className="flex flex-col bg-[#FDF8F3]" style={{ minHeight: '100dvh' }}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              <span className={cn('rounded-full px-2.5 py-0.5 text-xs font-bold', statoBadge.cls)}>
                {statoBadge.label}
              </span>
              {animale && <span className="text-xs text-gray-400">{animale.nome}</span>}
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 px-5 pb-32 space-y-4">
        <div className="rounded-3xl bg-white border border-gray-100 shadow-sm px-5 py-2">
          ...
          {impegno.ora && <RigaInfo label="Ora" valore={impegno.ora.slice(0, 5)} />}
          <RigaInfo
            label="Ripetizione"
            valore={labelFrequenza[impegno.frequenza] ?? impegno.frequenza}
          />
          {impegno.note && <RigaInfo label="Note" valore={impegno.note} />}
        </div>
